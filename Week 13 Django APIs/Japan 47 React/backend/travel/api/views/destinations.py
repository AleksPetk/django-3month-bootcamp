...
from travel.models import Favorite, Place, PlaceImage, Review, ReviewVote, VisitedPlace
...
class PlaceViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action in {"create", "update", "partial_update", "destroy", "images", "gallery_image"}:
    ...
    def get_queryset(self):
        ...
        if self.action == "retrieve":
            queryset = queryset.prefetch_related(
                Prefetch("reviews", queryset=reviews), "gallery_images"
            )
    ...
    def images(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            if locked_place.gallery_images.count() >= MAX_PLACE_GALLERY_IMAGES:
                return JsonResponse({
                    "error": {
                        "code": "validation_error",
                        "message": "Please correct the highlighted fields.",
                        "fields": {"gallery_images": [f"A place can have up to {MAX_PLACE_GALLERY_IMAGES} gallery photos."]},
                    }
                }, status=400)
            serializer = PlaceImageSerializer(data=request.data, context={"request": request})
            serializer.is_valid(raise_exception=True)
            serializer.save(place=locked_place)
        return JsonResponse(serializer.data, status=201)

    @action(detail=True, methods=["delete"], permission_classes=[IsAuthenticated], url_path=r"images/(?P<image_id>[^/.]+)")
    def gallery_image(self, request, pk=None, image_id=None):
        place = self.get_object()
        if not (request.user.is_staff or place.author_id == request.user.id):
            return JsonResponse({"error": {"code": "permission_denied", "message": "Only the owner can remove gallery images."}}, status=403)
