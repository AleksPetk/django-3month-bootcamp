...
class PlaceDetailSerializer(PlaceListSerializer):
    ...
    class Meta(PlaceListSerializer.Meta):
        fields = PlaceListSerializer.Meta.fields + ("google_maps_url", "official_website", "travel_tips", "latitude", "longitude", "gallery_images", "reviews")
...
class PlaceWriteSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        request = self.context.get("request")
        if request and not request.user.is_staff and updated.status == Place.Status.PUBLISHED:
            updated.status = Place.Status.PENDING
            Place.objects.filter(pk=updated.pk).update(status=Place.Status.PENDING)
        return updated
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Profile
        fields = ("id", "username", "email", "nickname", "display_name", "profile_image", "profile_image_url", "joined_at", "stats", "places", "reviews", "is_owner", "follower_count", "following_count", "is_following", "favorites", "recent_activity")
