...
export default function SiteLayout() {
  ...
  useEffect(() => {
    ...
    const labels = { regions: 'Regions', prefectures: 'Prefectures', places: 'Places', search: 'Search', contributors: 'Contributor', 'my-travel': 'My travel', privacy: 'Privacy', terms: 'Terms' }
  ...
  return <div className="site-shell">
    ...
    <header className="site-header"><Link className="brand" to="/" onClick={close}><img src="/logo.PNG" alt="Japan 47" /></Link>
      ...
      <nav id="main-nav" className={open ? 'nav nav--open' : 'nav'} aria-label="Main navigation">
        ...
        {user ? <><NavLink onClick={close} to="/my-travel">My travel</NavLink><NavLink className="nav__user" onClick={close} to={`/contributors/${user.id}`}>{user.profile_image_url ? <img src={user.profile_image_url} alt="" /> : <span>{user.display_name[0]}</span>}{user.display_name}</NavLink><button onClick={() => { close(); logout() }}>Logout</button></> : <><NavLink onClick={close} to="/login">Login</NavLink><NavLink className="nav__accent" onClick={close} to="/register">Register</NavLink></>}
      </nav>
    </header>
    <main id="main"><Outlet /></main>
    <footer><nav aria-label="Legal"><Link to="/privacy">Privacy Policy</Link><Link to="/terms">Terms of Use</Link></nav><p>© {new Date().getFullYear()} Japan 47. All rights reserved.</p><small>Created by Aleks Petk</small></footer>
  </div>
}
