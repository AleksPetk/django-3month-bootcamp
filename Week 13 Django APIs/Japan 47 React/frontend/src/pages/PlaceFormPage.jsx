...
export default function PlaceFormPage() {
  ...
  useEffect(() => {
    if (editing && place?.id) {
      setValues({ name: place.name, description: place.description, city: place.city, google_maps_url: place.google_maps_url, official_website: place.official_website, travel_tips: place.travel_tips, best_season: place.best_season, latitude: place.latitude || '', longitude: place.longitude || '', prefecture_id: String(place.prefecture.id) })
  ...
  const keptGalleryCount = existingGallery.length - removedGalleryIds.length
  ...
  const toggleGalleryRemoval = (imageId) => {
    setRemovedGalleryIds((current) => current.includes(imageId) ? current.filter((idToKeep) => idToKeep !== imageId) : [...current, imageId])
    setGalleryFiles([])
    setErrors((current) => ({ ...current, gallery_images: undefined }))
  }
  const submit = async (event) => {
    ...
    if (removeImage && !image) body.append('remove_image', 'true')
    try {
      const result = await api(editing ? `/places/${id}/` : '/places/', { method: editing ? 'PATCH' : 'POST', body })
      for (const imageId of removedGalleryIds) await api(`/places/${result.id}/images/${imageId}/`, { method: 'DELETE' })
  ...
  return <section className="form-page"><div><p className="eyebrow">{editing ? 'Update your contribution' : 'Community contribution'}</p><h1>{editing ? 'Edit place' : 'Suggest a place'}</h1><p>{editing ? 'Material changes will return a published place to moderation.' : 'Your destination will be reviewed by the Japan 47 team before publication.'}</p></div><form onSubmit={submit} encType="multipart/form-data">{errors.general && <p className="form-error">{errors.general}</p>}
    ...
    <FormField label="Main image" name="image" errors={errors} hint="JPEG, PNG, WebP, or HEIF; maximum 8 MB.">
      {editing && place.image_url && !image && <div className={`existing-main-image${removeImage ? ' existing-main-image--removed' : ''}`}><MediaImage src={place.image_url} alt={`${place.name} current main`} /><div><strong>{removeImage ? 'Main image will be removed' : 'Current main image'}</strong><button type="button" className="button" onClick={() => setRemoveImage((current) => !current)}>{removeImage ? 'Keep image' : 'Remove image'}</button></div></div>}
      {image && <div className="selected-files"><span>New main image: {image.name}</span><button type="button" onClick={() => setImage(null)}>Clear selection</button></div>}
      <ImageDropInput id="image" onFiles={chooseMainImage} />
    </FormField>
    <FormField label="Gallery images" name="gallery_images" errors={errors} hint={`Up to ${MAX_GALLERY_IMAGES} additional photos. ${remainingGallerySlots} slot${remainingGallerySlots === 1 ? '' : 's'} available.`}>
      {editing && existingGallery.length > 0 && <div className="existing-gallery-editor">{existingGallery.map((galleryImage) => { const removed = removedGalleryIds.includes(galleryImage.id); return <article className={removed ? 'is-removed' : ''} key={galleryImage.id}><MediaImage src={galleryImage.thumbnail_url || galleryImage.image_url} alt={galleryImage.caption || `${place.name} gallery photo`} /><button type="button" onClick={() => toggleGalleryRemoval(galleryImage.id)}>{removed ? 'Keep photo' : 'Remove photo'}</button></article> })}</div>}
      {galleryFiles.length > 0 && <div className="selected-files"><span>{galleryFiles.length} new gallery photo{galleryFiles.length === 1 ? '' : 's'} selected</span><button type="button" onClick={() => setGalleryFiles([])}>Clear selection</button></div>}
      {remainingGallerySlots > 0 && <ImageDropInput id="gallery_images" multiple onFiles={chooseGalleryFiles} />}
    </FormField>
