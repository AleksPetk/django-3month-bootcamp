...
function PlaceSection({ title, data, empty }) {
  return <section className="feature"><header className="section-header"><div><h2>{title}</h2></div></header>{data?.length ? <div className="grid grid--3">{data.map((item) => <PlaceCard key={item.id} place={item.place} />)}</div> : <EmptyState message={empty} />}</section>
}
