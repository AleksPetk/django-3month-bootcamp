...
export default function PrefectureDetailPage() {
  ...
  return <article className="detail page prefecture-detail"><p className="breadcrumbs"><Link to="/regions">Regions</Link> / <Link to={`/regions/${data.region.name}`}>{data.region.label}</Link> / {data.name}</p>
    <header className="prefecture-hero">
      <div className="prefecture-hero__image"><MediaImage src={data.image_url} alt={`${data.name} prefecture`} mark="県" priority /><span>{data.region.label} · Japan</span></div>
      <div className="prefecture-hero__content"><p className="eyebrow">Discover the prefecture</p><h1>{data.name}</h1><Rating value={data.average_rating} large /><p className="prefecture-intro">{data.description || 'Community recommendations for this prefecture are growing.'}</p><div className="prefecture-hero__actions"><Link className="button button--primary" to={`/places?prefecture=${encodeURIComponent(data.name)}`}>Explore places</Link>{user && <Link className="button" to={`/places/new?prefecture=${encodeURIComponent(data.name)}`}>Suggest a place</Link>}</div></div>
    </header>
