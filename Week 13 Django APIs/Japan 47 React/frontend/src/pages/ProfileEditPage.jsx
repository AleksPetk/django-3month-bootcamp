...
export default function ProfileEditPage() {
  const { user, loading, reloadUser } = useAuth(); const navigate = useNavigate(); const [values, setValues] = useState(null); const [image, setImage] = useState(null); const [errors, setErrors] = useState({}); const [busy, setBusy] = useState(false)
  ...
  const submit = async (e) => { e.preventDefault(); setBusy(true); const body = new FormData(); body.append('nickname', form.nickname); body.append('email', form.email); if (image) body.append('profile_image', image); try { await api('/profile/', { method: 'PATCH', body }); await reloadUser(); navigate(`/contributors/${user.id}`) } catch (err) { setErrors(err.fields || { general: err.message }) } finally { setBusy(false) } }
