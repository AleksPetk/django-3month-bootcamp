...
export default function PlaceDetailPage() {
  ...
  const toggleState = async (kind, active) => { await api(`/places/${data.id}/${kind}/`, { method: active ? 'DELETE' : 'POST' }); setRevision((value) => value + 1) }
  ...
  return <article className="detail page"><p className="breadcrumbs"><Link to="/places">Places</Link> / <Link to={`/prefectures/${data.prefecture.name}`}>{data.prefecture.name}</Link> / {data.name}</p>
    ...
    <div className="owner-actions">{user && <><button className="button" onClick={() => toggleState('favorite', data.is_favorite)}>{data.is_favorite ? 'Saved ♥' : 'Save place ♡'}</button><button className="button" onClick={() => toggleState('visited', data.is_visited)}>{data.is_visited ? 'Visited ✓' : 'Mark visited'}</button></>}<button className="button" onClick={share}>Share</button>{data.can_edit && <><Link className="button" to={`/places/${data.id}/edit`}>Edit place</Link><button className="button button--danger" onClick={remove}>Delete</button></>}</div>
